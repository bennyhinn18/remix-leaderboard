...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const newEvent = {
      ...
      clan_id: formData.get('clan_id') as string,
      leading_clan: {
...
export default function NewEvent() {
  ...
  useEffect(() => {
    if (selectedClan) {
      const selectedClanData = clans.find(
        (clan) => clan.clan_name === selectedClan
      );
      if (selectedClanData) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedClan) errors.clan = 'Clan selection is required';
    ...
    agendaItems.forEach((item, index) => {
      ...
      if (!item.description)
        errors[`agenda[${index}].description`] = 'Description is required';
    });
    ...
    // Automatically add clan data to form data
    const selectedClanData = clans.find(
      (clan) => clan.clan_name === selectedClan
    );
    if (selectedClanData) {
      formData.set('clan_id', selectedClanData.id.toString());
      formData.set('clanScore', selectedClanData.clan_score.toString()); // Add this line
    }
    ...
    agendaItems.forEach((item, index) => {
      ...
      if (item.speaker)
        formData.append(`agenda[${index}].speaker`, item.speaker);
    });

    const result = (await submit(formData, { method: 'POST' })) as unknown as {
      success?: boolean;
    };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  ...
                  style={{
                    borderWidth: 1,
                    borderColor: validationErrors.date ? undefined : 'white',
                  }}
                >
      ...
      <div className="grid gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label htmlFor="clanName" className="text-white">
              Clan Name
            </Label>
            <select
              id="clanName"
          ...
          <div>
            <Label htmlFor="clanScore" className="text-white">
              Clan Score
            </Label>
            <Input
              id="clanScore"
