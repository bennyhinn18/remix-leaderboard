...
import {
  ...
  Outlet,
  useNavigation,
} from 'react-router-dom'; // using react-router for navigation in CSR
...
export default function Events() {
  ...
  const [joinedEvents, setJoinedEvents] = useLocalStorage<string[]>(
    'joinedEvents',
    []
  );
  ...
  const handleStatusFilter = useCallback(
    (status) => {
      ...
      setSelectedEvent(
        events.find((e) => (status === 'all' ? true : e.status === status))
      );
    },
    [events]
  );
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        {location.pathname !== '/events/new' && (
          <>
            <div className="flex justify-between items-center">
              <h1 className="text-5xl font-bold text-white text-center w-full mt-6">
                Weekly Events
              </h1>
              {isOrganiser && (
                <Button
            ...
            <AnimatePresence mode="popLayout">
              ...
              >
                <WeekAnnouncement
                  leadingClan={selectedEvent?.leading_clan}
                  isLoading={false}
        ...
        {location.pathname === '/events/new' && (
          <div
            ...
            className="mt-6 p-6 rounded-lg shadow-lg border border-blue-700 bg-blue-800/50"
          >
