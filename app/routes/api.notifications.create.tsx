...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!organiserStatus) {
    return json(
      { success: false, error: "Unauthorized. Only organizers can send notifications." },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (memberIdsString) {
      memberIds = memberIdsString.split(",").map(id => parseInt(id.trim(), 10));
    }
    ...
    return json(
      { success: false, error: "An error occurred while creating the notification" },
      { status: 500 }
    );
  }
}
