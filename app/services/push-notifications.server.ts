...
export async function sendPushNotificationToMember(
  ...
  try {
    ...
    if (!subscriptions || subscriptions.length === 0) {
      return { success: false, error: "No push subscriptions found for this member" };
    }
...
export async function sendPushNotificationBroadcast(
  ...
  try {
    ...
    return { success: false, error: error.message || "Failed to send broadcast push notification" };
  }
}
