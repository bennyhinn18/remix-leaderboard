              ...
              >
                <motion.span
                  ...
                  animate={{
                    ...
                    color: isActive ? "rgb(96, 165, 250)" : "rgb(156, 163, 175)",
                  }}
                ...
                <span className={cn("text-xs font-medium", isActive ? "text-blue-400" : "text-gray-400")}>
                  {item.label}
                </span>
    ...
    >
      {navItems.map((item) => {
        ...
        <Link
          key={item.href}
          to={item.href}
          className={cn("relative group flex items-center", "transition-all duration-300 ease-in-out")}
          aria-label={item.label}
        >
          <motion.div
            className={cn(
            ...
            isActive ? `${item.gradientFrom} ${item.gradientTo}` : "from-gray-800/50 to-gray-900/50",
            )}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <Icon className={cn("w-6 h-6", isActive ? item.color : "text-gray-400 group-hover:text-white")} />
