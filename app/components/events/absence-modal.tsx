        ...
        <AnimatePresence mode="wait">
          {!isSent ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <fetcher.Form method="post" action="/api/send-absence-notification" className="space-y-4">
                {fetcher.data?.error && (
                ...
                <div className="space-y-2">
                  <Input
                    ...
                    aria-invalid={!validateEmail(email) && email ? "true" : undefined}
                    disabled={isSending}
                  />
                ...
                <div className="space-y-2">
                  <Textarea
                    ...
                    aria-invalid={reason && reason.length < 10 ? "true" : undefined}
                    disabled={isSending}
                  />
                  {reason && reason.length < 10 && (
                    <p className="text-sm text-red-400">Please provide a reason (minimum 10 characters)</p>
                  )}
                </div>
            ...
            >
              ...
              <div className="mt-4 text-sm text-blue-300">A confirmation has been sent to {email}</div>
            </motion.div>
          )}
        </AnimatePresence>
      </DialogContent>
