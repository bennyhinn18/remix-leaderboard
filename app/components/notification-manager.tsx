...
import { Bell, AlertCircle, Calendar, Trophy, Info, Send, UsersRound, User } from "lucide-react";
...
export function NotificationManager({ allMembers, recentNotifications }: NotificationManagerProps) {
  ...
  const handleSubmit = () => {
    ...
    if (!isBroadcast && selectedMembers.length === 0) {
      newErrors.members = "Please select at least one member or enable broadcast";
    }
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case "urgent":
        return <Badge className="bg-red-500/10 text-red-400 border-red-500/30">Urgent</Badge>;
      case "high":
        return <Badge className="bg-orange-500/10 text-orange-400 border-orange-500/30">High</Badge>;
      case "normal":
        return <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/30">Normal</Badge>;
      case "low":
        return <Badge className="bg-gray-500/10 text-gray-400 border-gray-500/30">Low</Badge>;
      default:
        return <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/30">Normal</Badge>;
    }
  };
        ...
        <DialogContent className="bg-gray-900 border border-gray-800 text-white max-w-3xl">
          ...
          <Tabs defaultValue="create" value={currentTab} onValueChange={setCurrentTab}>
            ...
            <TabsContent value="create" className="border-none p-0 mt-4">
              <fetcher.Form method="post" action="/api/notifications/create" className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setNotificationData({...notificationData, category: value})}
                      >
                  ...
                  <div className="bg-gray-800 p-4 rounded-lg">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <Checkbox
                        id="broadcast"
                        checked={isBroadcast}
                        onCheckedChange={(checked) => setIsBroadcast(checked === true)}
                      />
                      <label
                        htmlFor="broadcast"
            ...
            <TabsContent value="recent" className="border-none p-0 mt-4 max-h-[60vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  {recentNotifications.map((notification) => (
                    ...
                    >
                      ...
                      <p className="text-gray-300 text-sm mb-3">{notification.content}</p>
                      
                      <div className="flex justify-between text-xs text-gray-400">
                        <span>
                          Sent {formatDistanceToNow(new Date(notification.created_at), { addSuffix: true })}
                        </span>
                        {notification.is_broadcast ? (
                          <Badge variant="outline" className="text-[10px] py-0 h-4 bg-blue-500/10 border-blue-500/30">
                            Broadcast
                          </Badge>
                        ) : (
                          <span>Sent to {notification.recipient_count || "..."} members</span>
                        )}
                      </div>
                    </div>
                  ))}
