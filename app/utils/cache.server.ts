...
export class Cache<T> {
  ...
  constructor(defaultTTLSeconds: number = 60) { // Default TTL of 60 seconds
...
export function getCacheKeyFromRequest(request: Request, suffix: string = ''): string {
...
export async function getCachedMember(request: Request, username: string, supabase?: any) {
  ...
  const supabaseClient = supabase || createServerSupabase(request, new Response());
...
export async function getCachedPoints(request: Request, memberId: number, supabase?: any) {
